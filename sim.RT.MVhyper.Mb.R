...
sim.RT.MVhyper.Mb <-
  function(N=NA,lam0.p=NA,lam0.c=NA,sigma=NA,theta.d=NA,p0.p=NA,p0.c=NA,lambda=NA,K=10,X=NA,buff=NA,obstype="poisson",
           n.select=NA,K2D=NA){
    library(abind)
    ...
    if(obstype=="bernoulli"){
      ...
      lamd.p <- lam0.p*exp(-D*D/(2*sigma*sigma))
      lamd.c <- lam0.c*exp(-D*D/(2*sigma*sigma))
      for(i in 1:N){
        for(j in 1:J){
          for(k in 1:K){
            if(y.state[i,j,k]==0){ #1st cap
              y.true[i,j,k] <- rpois(1,lamd.p[i,j]*K2D[j,k])
            }else{ #subsequent cap
              y.true[i,j,k] <- rpois(1,lamd.c[i,j]*K2D[j,k])
            }
            if((y.true[i,j,k]>0)&(k<K)){
              y.state[i,j,(k+1):K] <- 1
            }
          }
        }
      ...
      for(i in 1:N){
        for(j in 1:J){
          for(k in 1:K){
            if(K2D[j,k]>0){
              if(y.state[i,j,k]==0){ #1st cap
                y.true[i,j,k] <- rnbinom(1,mu=lamd.p[i,j],size=theta.d)
              }else{
                y.true[i,j,k] <- rnbinom(1,mu=lamd.c[i,j],size=theta.d)
              }
              if((y.true[i,j,k]>0)&(k<K)){
                y.state[i,j,(k+1):K] <- 1
              }
      ...
      if(is.na(p0.p))stop("must provide p0.p for siteUse_ZTpois obstype")
      if(is.na(p0.c))stop("must provide p0.c for siteUse_ZTpois obstype")
      ...
      for(i in 1:N){
        for(j in 1:J){
          for(k in 1:K){
            if(K2D[j,k]>0){
              if(y.state[i,j,k]==0){ #1st cap
                y.det[i,j,k] <- rbinom(1,1,pd.p[i,j])
              }else{ #subsequent
                y.det[i,j,k] <- rbinom(1,1,pd.c[i,j])
              }
              if(y.det[i,j,k]==1){ #no trap response on the counts
                y.true[i,j,k] <- rzapois(1,lambda,pobs0=0)
              }
              if((y.det[i,j,k]>0)&(k<K)){#trap response to detection
                y.state[i,j,(k+1):K] <- 1
              }
            }
          }
