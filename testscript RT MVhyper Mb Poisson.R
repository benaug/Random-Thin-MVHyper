library(nimble)
library(coda)
source("sim.RT.MVhyper.Mb.R")
source("NimbleModelRT MVhyper Mb Poisson.R")
source("NimbleFunctionsRT MVhyper Mb Poisson.R")
source("init.RT.MVhyper.Mb.R")
source("sSampler.R")
...
data=sim.RT.MVhyper.Mb(N=N,lam0.p=lam0.p,lam0.c=lam0.c,sigma=sigma,K=K,X=X,buff=buff,n.select=n.select,obstype="poisson")
...
inits=list(lam0.p=1,lam0.c=1,sigma=1) #ballpark inits to build data
...
Niminits <- list(z=nimbuild$z,s=nimbuild$s,ID=nimbuild$ID,capcounts=rowSums(nimbuild$y.true),
                 y.true=nimbuild$y.true,y.state=nimbuild$y.state,
                 sigma=inits$sigma,lam0.p=inits$lam0.p,lam0.c=inits$lam0.c)
...
#replace independent lam0.p and sigma samplers with block sampler better accommodating for posterior covariance
...
conf$removeSampler(c("lam0.p","lam0.c","sigma"))
conf$addSampler(target = c("lam0.p","lam0.c","sigma"),
                  type = 'RW_block',
